...
class AppServer:
    ...
    async def terminal_speech_result(self, payload: dict[str, Any]) -> dict[str, Any]:
        response = {"type": "tts-result", "requestId": payload["requestId"]}
        text = payload.get("text")
        if not isinstance(text, str) or not text.strip() or len(text) > TTS_MAX_TEXT_CHARS:
            return {**response, "error": "Invalid text."}
        try:
            ...
            return {**response, "error": "Speech service unavailable."}
        # Terminal output owns binary frames; speech stays a correlated JSON reply.
        return {**response, "contentType": "audio/wav", "audio": base64.b64encode(audio).decode("ascii")}
    ...
    async def websocket_handler(self, connection: ServerConnection) -> None:
        ...
        speech_tasks: dict[str, asyncio.Task] = {}

        async def send_speech_result(payload: dict[str, Any]) -> None:
            # A tab switch can replace the bridge while synthesis is in flight.
            while True:
                current_bridge = bridge
                async with current_bridge.send_lock:
                    if current_bridge is not bridge:
                        continue
                    await self.send_json(connection, payload)
                    return

        async def synthesize_speech(payload: dict[str, Any]) -> None:
            try:
                await send_speech_result(await self.terminal_speech_result(payload))
            except ConnectionClosed:
                pass
            finally:
                speech_tasks.pop(payload["requestId"], None)
        ...
        try:
            ...
            while True:
                ...
                if msg_type == "tts-request":
                    ...
                    if not isinstance(request_id, str) or not 1 <= len(request_id) <= 80:
                        continue
                    if request_id in speech_tasks:
                        continue
                    if len(speech_tasks) >= 2:
                        await send_speech_result({
                            "type": "tts-result", "requestId": request_id,
                            "error": "Speech service busy. Try again.",
                        })
                        continue
                    speech_tasks[request_id] = asyncio.create_task(synthesize_speech(payload))
                    continue
            ...
            pending_speech = list(speech_tasks.values())
            for task in pending_speech:
                task.cancel()
            await asyncio.gather(*pending_speech, return_exceptions=True)
            receive_task.cancel()
            tab_task.cancel()
